...
export const getContractByAddressValidation = (
  ...
  const schema = Joi.object({
    address: Joi.string()
      ...
      .messages({
        "custom.validHash": `contract address should be start with ${environment.addressPrefix}`,
        "custom.hashLength": "contract address length should be equals to 64",
        "custom.noLeadingSpaces":
          "contract address should not start with a space",
        "string.pattern.base": `contract address must start with ${environment.addressPrefix}`,
