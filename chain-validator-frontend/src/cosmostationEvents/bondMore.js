...
export const cosmosStationBondMore = async (
  userAddress,
  valoperAddress,
  stakeAmount,
  setIsLoading,
  ...
  if (window?.cosmostation?.providers?.keplr) {
    try {
      ...
      const gasUsed = await simulateMsgs(
        ...
        key.bech32Address,
        [protoMsgs],
        [{ denom: ChainConfig.currencies[0].coinMinimalDenom || (CURRENCY || '').toLowerCase(), amount: "0" }]
      );
      ...
      if (e instanceof Error) {
        if (e.message === "Request rejected") {
          ...
          toast.error(friendlyMessage || TOAST_MESSAGES.FAILED_TO_BOND_MORE);
          navigate(ROUTE_PATHS.DASHBOARD);
        }
      } else {
